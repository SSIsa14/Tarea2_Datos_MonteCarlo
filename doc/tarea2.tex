...
\input{datos_parte1}
\input{datos_integrales}
\input{datos_dimension}
\input{datos_varianza}
\input{datos_randu}
...
Primero se hizo una investigación personal sobre cada uno de los temas y
sobre lo que pedía el enunciado. Las fuentes encontradas se le compartieron a
la herramienta para que le sirvieran de base, y se le pidió además que
ampliara la búsqueda en los puntos donde hiciera falta.

Para el código se le entregaron las semillas específicas que debían usarse,
junto con las opciones disponibles, y ejemplos de código genérico de cómo se
resuelve cada una de las secciones. Sobre esa base se le pidió apoyo para
generar las implementaciones correspondientes.

Los resultados obtenidos se analizaron de forma personal, y esos análisis se
le entregaron en borrador para que ayudara a mejorar la redacción antes de
incorporarlos al documento.

Después se le pidió ayuda con el código del documento en \LaTeX, tomando en
cuenta todo lo que se había compartido previamente.

Por último se hizo una revisión del documento completo para mejorar su
estructura, la redacción de algunas partes y las conclusiones.
...
\subsubsection{Chi-cuadrado} Se parte $[0,1)$ en $k=10$ celdas de igual ancho.
...
\subsubsection{Kolmogorov--Smirnov} Compara la función de distribución empírica
...
\subsubsection{Autocorrelación de retardo 1} Es la primera prueba de
...
\subsubsection{Prueba de rachas} Codifica cada valor como ``arriba'' o ``abajo''
...
\paragraph{Comportamiento de los bits bajos} Con $m = 2^{32}$ hay otro
...
estructura. Es importante no malinterpretarlo: eso no quiere decir que la retícula no
...
\paragraph{Razón de la falta de garantía de período} La diferencia de
...
\paragraph{El cero como estado absorbente} Hay un modo de falla todavía
...
\paragraph{Comportamiento con la semilla usada} Con $k = 8$ dígitos y semilla
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.46\textwidth]{cubo_MiddleSquare.png}
\caption{Ternas consecutivas $(u_i, u_{i+1}, u_{i+2})$ del método de
cuadrados medios en el cubo $[0,1]^3$.}
\label{fig:cuboCM}
\end{figure}

La figura~\ref{fig:cuboCM} muestra el efecto del colapso sobre las ternas.
Como el \CMFracCeros{} de la muestra son ceros, casi todos los puntos se
amontonan en el origen del cubo, y lo único que queda repartido es el rastro
del tramo útil, es decir, los valores que alcanzó a producir antes de morir.
...
\paragraph{Origen del período} Toda la iteración es una aplicación
...
\paragraph{Ausencia de seguridad criptográfica} Con todo lo bueno que tiene, MT19937 no sirve para
...
\paragraph{Razón de la condición sobre los primos}
...
\paragraph{Hallazgo sobre la elección de los primos}
Al implementar el generador apareció un resultado que no estaba previsto en
el planteamiento y que vale la pena documentar, porque cambia la manera de
elegir los parámetros: la condición $p \equiv q \equiv 3 \pmod 4$ es
necesaria pero \textbf{no suficiente} para tener buen período. El período de
la órbita de~\eqref{eq:bbs} es
\begin{equation}
...
La primera elección de parámetros fueron los primos de Mersenne
$p = 2^{31}-1$ y $q = 2^{61}-1$, que cumplen la condición y son los
candidatos más naturales por lo conocidos que son. Con ellos el generador
recorrió un ciclo de apenas unos 30 estados: de los 64 valores posibles de un
bloque de 6 bits sólo llegaron a aparecer 44, y las cuatro pruebas rechazaron
$H_0$ de forma contundente. Al contrastar ese comportamiento contra la
teoría quedó clara la causa: $p-1$ y $q-1$ se factorizan en primos pequeños,
de modo que $\lambda(\lambda(N))$ resulta pequeño y con él el orden de 2. Ser
primo no alcanza; lo que determina el período es cómo se factoriza el número
anterior.

El ajuste que resuelve el problema es exigir \emph{primos seguros}, es decir
$p = 2p'+1$ con $p'$ también primo, lo que obliga a $\lambda(N)$ a tener un
factor primo grande. La implementación final usa
\[
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.46\textwidth]{cubo_BlumBlumShub.png}
\caption{Ternas consecutivas $(u_i, u_{i+1}, u_{i+2})$ de Blum Blum Shub en
el cubo $[0,1]^3$.}
\label{fig:cuboBBS}
\end{figure}

En la figura~\ref{fig:cuboBBS} las ternas llenan el cubo sin ninguna
estructura visible. A diferencia de los generadores congruenciales, aquí no
hay una recurrencia lineal entre términos consecutivos que pueda confinarlas
a una retícula: el siguiente estado sale de un cuadrado modular, y
recuperarlo a partir de las salidas equivale al problema de la residuosidad
cuadrática.
...
\paragraph{Origen del defecto} El problema es algebraico y exacto. Escribiendo
$a = 65539 = 2^{16}+3$:
\begin{align}
...
\paragraph{Número de planos} La ecuación~\eqref{eq:randuplanos} dice que toda
...
\scriptsize
\begin{tabular}{L{2.1cm}|L{2.4cm}|L{2.8cm}|L{2.6cm}|L{3.4cm}}
\toprule
...
\input{tabla_velocidad}

Es importante precisar qué mide la tabla~\ref{tab:velocidad}: es el costo en Python
...
\paragraph{Independencia de la tasa respecto a la dimensión} Repitamos el
...
\input{tabla_integrales}

\subsubsection{Lectura de los resultados} Las seis estimaciones quedan a menos
...
\input{tabla_pendientes}

\subsubsection{Estudio de convergencia} La figura~\ref{fig:convergencia} barre
...
\subsubsection{Comparación entre generadores} Este resultado hay que subrayarlo
...
\subsubsection{Interpretación geométrica} La
...
\subsubsection{Comparación contra la rejilla} Con el mismo presupuesto de
...
\paragraph{Equivalencia en número de muestras} Un factor de reducción $F$ quiere
...
\paragraph{Alcance y límites de cada técnica} No, y cada técnica falla por
...
\subsubsection{Resultado con la región grande} Con el octante de la bola
...
\subsubsection{Resultado con la región pequeña} Con la bola de radio
...
\subsubsection{Explicación geométrica del sesgo} La respuesta está en las dos
gráficas de ternas en 3D de la Parte 1. En la figura~\ref{fig:cuboRANDU}, que
es la nube vista desde un ángulo cualquiera, los puntos parecen llenar el
cubo $[0,1]^3$ sin ninguna estructura. En la figura~\ref{fig:planos}, que es
la misma nube girada hasta mirar en la dirección del vector normal
$(9,-6,1)$, se ve lo que realmente pasa: las ternas de RANDU no viven en el
cubo, viven en \PlanosRandu{} planos paralelos. Y es exactamente de esas
ternas de donde salen los puntos $(x,y,z)$ que usa el estimador de esta
sección. Entonces el estimador de Monte Carlo no está midiendo el volumen de
la región: está midiendo qué fracción de \emph{esos planos} cae dentro de la
región, que es otra cosa.
...
\subsubsection{Efecto sobre los intervalos de confianza} La última
...
\subsubsection{Conexión con la prueba espectral de la Parte 1} Con esto se cierra el
